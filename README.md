...
Perplexity 팩트팩에는 매번 고유한 ID가 붙습니다. Gemini는 답변 첫 줄에 같은 팩트팩 ID를 출력해야 하며, ID가 없으면 앱이 팩트팩 미반영으로 판단해 한 번 자동 재질문합니다. Perplexity 답변이 지나치게 짧으면 앱이 전체 답변 복사를 시도하고 한 번 재검색합니다.
